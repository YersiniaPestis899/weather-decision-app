...
def main():
    ...
    if not st.session_state.aws_authenticated:
        st.subheader("AWS認証")
        st.warning("このアプリケーションはあなたのAWS認証情報を使用します。セキュリティに注意してください。")
        ...
        if st.button("認証"):
            ...
            if aws_client:
                ...
                st.success("AWS認証に成功しました！")
                st.experimental_rerun()
            else:
                st.error("AWS認証に失敗しました。")
    else:
